...
class KnowledgeGraphRepository(Protocol):
    ...
    ``add_node``/``add_edge`` upsert by id, so re-ingesting canonical knowledge
    updates rather than duplicates (the Knowledge Platform derives ids from
    entity slugs for exactly this reason).
    """
